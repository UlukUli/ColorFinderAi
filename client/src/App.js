...
const LANG = {
  ru:{
    ...
    generateBtn:"Сгенерировать случайную тему 🎲",generating:"Генерирую…",
    uploadBtn:"↓  Загрузить изображение",analyzing:"Обработка…",
    dragImage:"Перетащите изображение",orClick:"или нажмите для загрузки.",
    savePalette:"♡  Сохранить палитру",themeLabel:"Тема: ",
    ...
    savedCount:n=>`Сохранённые палитры (${n})`,logout:"Выйти из аккаунта",
    aiThinking:"AI думает…",pickerHint:"Нажми, чтобы\nвыбрать цвет",
    ...
    switchToReg:"Нет аккаунта? Зарегистрироваться",switchToLog:"Уже есть аккаунт? Войти",
    errUser:"Пользователь не найден",errPass:"Неверный пароль",
    errExists:"Имя уже занято",errFields:"Заполните все поля",
    palette:"Палитра",signIn:"Войти",
    clickToCopy:"Клик — скопировать",
    expandTitle:"Палитра",close:"✕ Закрыть",
  },
  en:{
    screens:[
    ...
    generateBtn:"Generate random theme 🎲",generating:"Generating…",
    uploadBtn:"↓  Upload image",analyzing:"Processing…",
    dragImage:"Drag image here",orClick:"or click to upload.",
    savePalette:"♡  Save palette",themeLabel:"Theme: ",
    emptyTitle:"Collection is empty",
    emptySub:"Save palettes while working\nto see them here.",
    savedPalettes:"Saved palettes",
    savedCount:n=>`Saved palettes (${n})`,logout:"Log out",
    aiThinking:"AI thinking…",pickerHint:"Click to\npick a color",
    ...
    switchToReg:"No account? Sign up",switchToLog:"Already have one? Log in",
    errUser:"User not found",errPass:"Wrong password",
    errExists:"Username taken",errFields:"Fill in all fields",
    palette:"Palette",signIn:"Sign in",
    clickToCopy:"Click to copy",
    expandTitle:"Palette",close:"✕ Close",
  },
};
...
function extractHex(t){return[...new Set([...t.matchAll(/([0-9A-Fa-f]{6})/g)].map(m=>m[1].toUpperCase()))].slice(0,16);}
const rgbToHex=(r,g,b)=>"#"+[r,g,b].map(x=>Math.min(255,Math.max(0,x)).toString(16).padStart(2,"0")).join("").toUpperCase();
...
const extractColorsFromImage = (imgElement) => {
  ...
  while(selected.length < 7) {
      selected.push(rgbToHex(Math.floor(Math.random()*255), Math.floor(Math.random()*255), Math.floor(Math.random()*255)));
  }
  return selected;
};
...
function AuthModal({t,users,setUsers,onClose,onLogin}){
  ...
  const submit=()=>{
    ...
    if(mode==="login"){
      ...
      if(!u){setErr(t.errUser);return;}if(u.password!==pw){setErr(t.errPass);return;}onLogin(un);
    }else{
      if(users.find(u=>u.username===un)){setErr(t.errExists);return;}
      setUsers(p=>[...p,{username:un,password:pw}]);onLogin(un);
    }
  };
  return(
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e=>e.stopPropagation()}>
        ...
        <input className="modal-input" placeholder={t.usernameLabel} value={un} onChange={e=>{setUn(e.target.value);setErr("");}}/>
        ...
        <button className="modal-btn" onClick={submit}>{mode==="login"?t.loginBtn:t.registerBtn}</button>
...
function ImageWithEyedropper({src,onRemove,clickToCopy}){
  const wrapRef=useRef(null);const canvasRef=useRef(null);const imgRef=useRef(null);
  const[hoverColor,setHoverColor]=useState(null);const[hoverPos,setHoverPos]=useState({x:0,y:0});
  const[copied,setCopied]=useState(false);
  const draw=useCallback(()=>{const img=imgRef.current;const c=canvasRef.current;if(!img||!c)return;c.width=img.naturalWidth;c.height=img.naturalHeight;c.getContext("2d").drawImage(img,0,0);},[]);
  const getPixel=useCallback(e=>{
    const wrap=wrapRef.current;const img=imgRef.current;const c=canvasRef.current;
    if(!wrap||!img||!c||!img.naturalWidth)return null;
    const rect=wrap.getBoundingClientRect();const ir=img.naturalWidth/img.naturalHeight;const wr=rect.width/rect.height;
    let iw,ih,ix,iy;
    ...
    const px=Math.floor((mx-ix)/iw*img.naturalWidth),py=Math.floor((my-iy)/ih*img.naturalHeight);
  ...
  const onClick=useCallback(e=>{
    ...
    const res=getPixel(e);if(res){navigator.clipboard.writeText(res.hex).then(()=>{setCopied(true);setTimeout(()=>setCopied(false),1400);});}
  },[getPixel]);
  return(
...
function App(){
  ...
  const handleChangeScreen=i=>{setColors(defaultColors);setImagePreview(null);setRandomTheme("");setCurrentScreen(i);if(fileInputRef.current)fileInputRef.current.value="";};
  const handleRemoveImage=e=>{e.stopPropagation();setImagePreview(null);setColors(defaultColors);if(fileInputRef.current)fileInputRef.current.value="";};
  
  const saveCurrent=name=>{
    if(isDefault)return;
    const dateStr = new Date().toLocaleDateString("ru-RU", {day: '2-digit', month: '2-digit', year: '2-digit'});
    setSavedPalettes(p=>[{id:Date.now(),name:name||t.palette,colors:[...colors],date:dateStr},...p].slice(0,30));
    showToast("✓ "+t.palette);
  };
  const deletePalette=(id,e)=>{e.stopPropagation();setSavedPalettes(p=>p.filter(x=>x.id!==id));};

  const updatePaletteName = (id, newName) => {
    setSavedPalettes(prev => prev.map(p => p.id === id ? { ...p, name: newName } : p));
  };
  ...
  const handleGenerateText=async()=>{
    ...
    try{const raw=await askClaude(`Theme: "${searchText}". Give 7 HEX colors.`,"Return ONLY 7 hex codes without # separated by spaces.");const c=extractHex(raw);if(c.length>=4)setColors(c.slice(0,7).map(h=>"#"+h));else throw new Error();}
  ...
  const handleGenerateRandom=async()=>{
    ...
    try{const raw=await askClaude("Invent a random creative theme and 7 colors.",`Return JSON: {"theme":"Theme in Russian","colors":"HEX HEX HEX HEX HEX HEX HEX"}. No extra text.`);
      const obj=JSON.parse(raw.replace(/```json|```/g,"").trim());setRandomTheme(obj.theme||"Случайная тема");
      const c=extractHex(obj.colors||"");if(c.length>=4)setColors(c.slice(0,7).map(h=>"#"+h));else throw new Error();}
    catch{const themes=["Космический закат","Неоновый киберпанк","Снежный лес","Утренняя мята","Глубокий океан"];
  ...
  const handleGenerateShades=hex=>{
    setBaseColor(hex);const r=parseInt(hex.slice(1,3),16),g=parseInt(hex.slice(3,5),16),b=parseInt(hex.slice(5,7),16);
    setColors(Array.from({length:16},(_,i)=>{const f=1.8-i*(1.8/15);
    ...
    <div className="app" data-theme={theme}>
      ...
      <header className="header">
        ...
        <div className="profile-area">
          ...
          <button className="lang-btn" onClick={()=>setLang(l=>l==="ru"?"en":"ru")}>{lang==="ru"?"EN":"RU"}</button>
          <div className="avatar-area" onClick={()=>!auth.isLoggedIn&&setAuth(a=>({...a,showModal:true}))}>
            <div className="avatar">
              {auth.isLoggedIn?<span className="avatar-letter">{auth.username[0].toUpperCase()}</span>:<span className="avatar-icon">👤</span>}
            </div>
        ...
        <div className="window">
          ...
          <div className="left-side">
            ...
            {currentScreen===0&&(
              <div className="input-row">
                ...
                <button className="go-btn" onClick={handleGenerateText} disabled={loading}>{loading?"…":"→"}</button>
              </div>
            )}
            ...
            {currentScreen===4&&(
              <div className="cabinet-menu">
                ...
                {auth.isLoggedIn && (
                  <button className="cabinet-menu-btn logout" onClick={() => setAuth({isLoggedIn:false,username:"",showModal:false})}>
          ...
          <div className="right-side">
            {currentScreen===1&&(
              <div className="screen02-layout"
                ...
                {imagePreview
                  ...
                  :<div className={`screen02-upload-zone${dragOver?" drag-active":""}`}
                ...
                <div className="screen02-vert-palette">
                  {loading?<div className="loading-bar-vert"><span>{t.aiThinking}</span></div>
            ...
            {currentScreen===4&&(
              savedPalettes.length===0
                ?<div className="cabinet-placeholder">
                    ...
                    <h2>{t.emptyTitle}</h2><p style={{whiteSpace:"pre-line"}}>{t.emptySub}</p>
                  </div>
                    ...
                    {savedPalettes.map(p=>(
                      <div key={p.id} className="saved-palette-card">
                        ...
                        <div className="card-bottom-info">
                          ...
                          <svg className="expand-action-icon" viewBox="0 0 24 24" onClick={e=>{e.stopPropagation();setExpandPalette(p);}}>
            ...
            {currentScreen!==4 && currentScreen!==2 && !isDefault&&(
              <button className="save-palette-btn" onClick={()=>saveCurrent(searchText||randomTheme||t.palette)}>
                {t.savePalette}
              </button>
            )}
          </div>
